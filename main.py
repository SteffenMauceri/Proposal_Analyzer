...
def generate_consolidated_summary(
    ...
    if services_run.get("Core Analysis", False):
        ...
        if analysis_items:  # Check if there are actual findings
            ...
            summary_prompt_parts.append(
                "- Core Proposal Q&A: Selected, but no findings were reported or an issue occurred."
            )
    ...
    if services_run.get("Spell Check", False):
        ...
        if spell_issues:  # Check if there are actual findings
            if len(spell_issues) == 1 and spell_issues[0].get("type") == "error_extraction":
                summary_prompt_parts.append(
                    "- Spell & Grammar Check: Failed to extract text from the document for spell checking."
                )
            else:
                ...
                actual_issues = [
                    ...
                    if issue.get("type") != "error_extraction" and issue.get("suggestion")
                ]
                num_spell_issues = len(actual_issues)
                if num_spell_issues > 0:
                    issue_types = list(
                        set(issue.get("type", "unknown") for issue in actual_issues)
                    )
                    summary_prompt_parts.append(
                    ...
                    summary_prompt_parts.append(
                        "- Spell & Grammar Check: No actionable spelling or grammar issues were identified (though the service ran)."
                    )
        else:
            summary_prompt_parts.append(
                "- Spell & Grammar Check: Selected, but no findings were reported or an issue occurred."
            )
